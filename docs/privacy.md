...
Essential records are limited to opaque identifiers, version/platform, timestamps, aggregate usage, stable error/action category, allow/confirm/block decision, request/run IDs, and revocation state. Optional content-free product telemetry is off until consent. Audit events expire on the operating schedule; active agent continuations expire within 30 minutes.
